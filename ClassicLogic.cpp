...
bool ClassicLogic::playMove(Point move, CellValue value) {
	...
	if((this->game_board_->getCellValue(move.getX()- 1, move.getY()) != BOEDER) &&
	   (this->game_board_->getCellValue(move.getX() - 1,move.getY()) != EMPTY) &&
	   (this->game_board_->getCellValue(move.getX() - 1,move.getY()) != value) &&
	...
	if ((this->game_board_->getCellValue(move.getX(),move.getY() - 1) != BOEDER) &&
		(this->game_board_->getCellValue(move.getX(),move.getY() - 1) != EMPTY) &&
		(this->game_board_->getCellValue(move.getX(),move.getY() - 1) != value) &&
	...
	// Check up right
	if ((this->game_board_->getCellValue(move.getX() + 1,move.getY() + 1) != BOEDER) &&
		(this->game_board_->getCellValue(move.getX() + 1,move.getY() + 1) != EMPTY) &&
		(this->game_board_->getCellValue(move.getX() + 1,move.getY()+ 1) != value) &&
    	...
    	for(int jndex = 1; jndex <= height; jndex++){
    		if(this->game_board_->getCellValue(index, jndex) == EMPTY) {
    			...
    			if((this->game_board_->getCellValue(index - 1,jndex) != BOEDER) &&
    			   (this->game_board_->getCellValue(index - 1,jndex) != EMPTY) &&
				   (this->game_board_->getCellValue(index - 1,jndex) != value) &&
				   (this->playDirection(index - 1, jndex, -1,0, value,true))) {
    				Point * p = new Point(index, jndex);
    				moves_list->push_back(p);
				// Check down
    			} else if((this->game_board_->getCellValue(index + 1,jndex) != BOEDER) &&
    					  (this->game_board_->getCellValue(index + 1,jndex) != EMPTY) &&
						  (this->game_board_->getCellValue(index + 1,jndex) != value) &&
						  (this->playDirection(index + 1, jndex, 1,0, value,true))) {
    				Point * p = new Point(index, jndex);
    				moves_list->push_back(p);
    			// Check left.
    			} else if ((this->game_board_->getCellValue(index,jndex - 1) != BOEDER) &&
    					   (this->game_board_->getCellValue(index,jndex - 1) != EMPTY) &&
					       (this->game_board_->getCellValue(index,jndex - 1) != value) &&
    			...
    			// Check right.
    				...
    				moves_list->push_back(p);
    				// Check up right
    			} else if ((this->game_board_->getCellValue(index + 1,jndex + 1) != BOEDER) &&
    					   (this->game_board_->getCellValue(index + 1,jndex + 1) != EMPTY) &&
						   (this->game_board_->getCellValue(index + 1,jndex + 1) != value) &&
